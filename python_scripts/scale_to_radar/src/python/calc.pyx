...
def calc_ref_rv(sio, bmap , radar , topo , min_dbz=-20., z=None ,  u=None , v=None , w=None , rho=None, qr=None, qs=None, qg=None, t=None, dryrun=False , use_wt=True ):
    ...
    dlon_=lon_-radar.longitude['data']
    dlat_=lat_-radar.latitude['data']

    mask_ = np.logical_not( np.logical_and( dlon_ == 0.0 , dlat_ == 0.0 ) )
    az_ = np.zeros( dlon_.shape )
    az_[mask_] = np.arctan2(dlon_[mask_]*np.cos(radar.latitude['data']*d2r)*d2r,dlat_[mask_]*d2r)
    ...
    cosd_ =np.sin( lat_ * d2r )*np.sin( radar.latitude['data'] * d2r ) + np.cos( lat_*d2r )*np.cos( radar.latitude['data'] * d2r )*np.cos( ( lon_ - radar.longitude['data'] )*d2r )
    cosd_[cosd_ > 1.0 ]=1.0
    cosd_[cosd_ <-1.0 ]=-1.0
    dist_ = np.arccos( cosd_ ) * re_

    dist_ = np.repeat(dist_[np.newaxis,:,:], nz, axis=0)
    elev_ = np.arctan2( z_ - radar.altitude['data'] , dist_ )
